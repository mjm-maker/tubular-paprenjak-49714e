...
4. **Background music** — choose **No music** or upload your own MP3, WAV, M4A or AAC up to 20 MB. Preview the uploaded song, set the voice and music volumes independently (voice 100%, music 15% by default), and remove or replace it at any time. A short song loops to cover the voice, a long one is trimmed to it, and both ends get a 1.2 s fade. Optional — the export works with no music at all.
...
GLASKO has no built-in song catalogue. A user-selected song is decoded in the browser, stays on that device and is mixed into the exported file locally. The UI reminds the user to upload only music they own or have permission to use, because social platforms may mute copyrighted audio.

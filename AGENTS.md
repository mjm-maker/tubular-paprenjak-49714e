...
- `app/page.tsx` is the only screen of the editor. It owns all state (audio source, background, animation, picture window and its artwork, headline, music selection and volumes, subtitle settings and cues, output format, watermark, export status, sharing) and composes the nine step panels plus the preview. It does **not** read the session: `components/AccountGate.tsx` mounts it only after Netlify Identity resolves a signed-in user, then the editor behaves exactly as before.
...
- `components/LiveCameraTest.tsx` is an explicitly non-broadcasting camera rehearsal for the planned Facebook LIVE flow. It opens the device camera and microphone locally, presents a vertical 9:16 camera canvas and reuses the ordinary Autocue script/settings from `localStorage`. It must continue to say that Facebook is not receiving video until a real WebRTC-to-RTMPS relay is connected; never turn the test label into a Go Live claim on UI alone.
...
Voice priority comes from the sidechain rather than a hidden slider ceiling. `buildDuckEnvelope(voice)` measures where speech actually is and pulls the bed down under it, while `effectiveMusicLevel(musicVolume, voiceVolume)` preserves every requested music step from 0% through 100%. The page builds the envelope once and passes it to `mixAudio` as `duck`, so the preview and the file cannot drift apart.
...
`lib/music.ts` owns the local-upload rules (20 MB; MP3/WAV/M4A/AAC, checked by extension *and* MIME type because Safari and Android pickers disagree about m4a and aac) and the shared 0–1 volume clamp. The product picker deliberately offers only **No music** and **Upload your song**. Do not put a built-in catalogue back into the UI unless explicitly requested, and never upload or bundle commercial music.

The picker auditions the uploaded song through one `<audio>` element owned by the page, driven by the id the panel highlights, so the button state and what you hear cannot disagree; starting an audition stops the main preview and vice versa. The audition obeys the same music-volume state instead of playing at a fixed level. Both volume sliders use `onInput`, larger phone targets and −/+ buttons, and the page also writes the changed value directly to the live `<audio>` element so a drag is audible immediately. "No music" is a card of its own rather than only a remove button, because voice-only should be visibly a choice.
...
Accounts are Netlify Identity (`@netlify/identity`) and are required to enter the editor. `components/AccountGate.tsx` leaves login, signup, recovery and reset routes public, shows a loading state while the session settles, and mounts every other page only for a signed-in user. `app/page.tsx` still does not read the session, and nothing about a video is attached to a user. No audio, image or MP4 is uploaded or stored, and there is **no database table for users** — Identity holds the credentials, hashes them, issues the JWT and sends the confirmation and recovery mail.

`lib/auth.ts` is deliberately the only code around it: field validation, one sentence of wording per failure, and the account date. Two rules there are not stylistic. A failed login never says which half was wrong, because saying so tells an attacker which addresses have accounts. A missing Identity service is worded as temporary unavailability, because the editor cannot open without it. **A password must never be stored, logged, put in a URL or written to `localStorage`**: it goes from the input straight into the Identity call. If you add a field to those forms, keep that property.

`components/AuthProvider.tsx` sits in the root layout rather than on the auth routes, because confirmation, recovery and invite links come back as a URL *hash* on whatever page the link opened — normally the editor — so `handleAuthCallback()` has to run there. `status` starts as `'loading'`, and `AccountGate` shows a neutral loading card until it resolves, so neither the editor nor a false logged-out screen flashes first.

The feature is enabled on deploy by the `netlify-identity` skill's activation script, which is why `.netlify/features/netlify-identity` exists. Identity's own settings (open vs invite-only registration, email confirmation) are project configuration, not code. There is no social login, plan or payment code. Signup requires name and unique email, accepts an optional phone in user metadata, and email confirmation decides whether an account enters the public member total.

`netlify/functions/member-count.mts` is the server-only count path. It uses `admin.listUsers()` with the operator token injected by the Netlify Functions runtime, paginates the complete Identity user set and returns **only** the number whose email is confirmed. Never send the operator token or any member field to the browser. `components/MemberCount.tsx` hides the line if that aggregate endpoint is unavailable.
...
- Six checks run without a browser, through `scripts/ts-resolve.mjs`, which lets plain `node` import the app's `.ts` modules: `npm run auth:check` (required signup fields, optional phone, confirmed-member pagination and public count parsing), `npm run music:check` (upload validation, loop/trim decisions and the complete audible volume range), `npm run subtitles:check` (cue timing against synthetic recordings), `npm run layout:check` (every element's box against every other, across all three formats), `npm run transcribe:check` (slice geometry, retries and partial recovery against a stubbed route) and `npm run export:check` (the real `mp4-muxer` driven with the exact options the encoder gives it, then the audio track read back out of the bytes — plus the acceptance rule itself, which must never approve a file whose sound was not heard). Run `auth:check` after touching the gate, account validation or member count; `music:check` after touching the picker, `lib/music.ts` or `lib/mix.ts`; the subtitle and layout checks after touching `lib/render.ts`, `lib/layout.ts`, `lib/picture.ts`, `lib/headline.ts`, `lib/align.ts` or `lib/analysis.ts`; the transcription check after touching `lib/transcribe.ts` or the transcribe route; and the export check after touching `lib/encode.ts`, `lib/mp4probe.ts` or `lib/mix.ts` — along with `npm run typecheck`.
...
- Accounts are required to enter the editor but still do nothing to the video itself. There is no plan, payment, profile photo or email change; a new signup must confirm by email if the project's Identity settings say so, and registration is open until set to invite-only there. Do not move recordings, images or exports into account storage.

...
- Three checks run without a browser, through `scripts/ts-resolve.mjs`, which lets plain `node` import the app's `.ts` modules: `npm run subtitles:check` (cue timing against synthetic recordings), `npm run layout:check` (every element's box against every other, across all three formats) and `npm run transcribe:check` (slice geometry, retries and partial recovery against a stubbed route). Run the first two after touching `lib/render.ts`, `lib/layout.ts`, `lib/picture.ts`, `lib/headline.ts`, `lib/align.ts` or `lib/analysis.ts`, and the third after touching `lib/transcribe.ts` or the transcribe route — along with `npm run typecheck`.
